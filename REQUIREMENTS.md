# VIM2 本地语音输入工具需求文档

## 0. 0.1.0 自包含安装版（覆盖下文旧 Portable-only 限制）

用户确认的 0.1.0 安装版仅供未签名内部测试：macOS 原生 Apple Silicon
`VIM2.app` + 含 Applications 链接的 `.dmg`；Windows 10/11 x64 原计划用
GitHub Actions `windows-2022` 构建 Inno Setup 当前用户 `Setup.exe`，但用户已
明确暂停并放弃本轮 Windows Actions / Setup 打包；本轮不生成或声称 Windows 产物。
Windows workflow 不得保留自动 `push` 触发：向 macOS 分支 push 不会自动运行
Windows job；只保留 `workflow_dispatch` 作为未来显式人工恢复入口，本轮不触发。
两端内置 Python 与平台依赖、**仅** Qwen3-ASR 0.6B INT8 CPU 模型；不要求
安装 Python、CUDA 或联网运行。安装程序只读数据包含固定六个模型文件和默认配置；
用户写入全部到 macOS `~/Library/Application Support/VIM2` 或 Windows
`%LOCALAPPDATA%\\VIM2` 的 config/runtime/temp。首次启动仅补缺、不覆写
现有设置和热词；锁、日志、编辑器与音频临时文件均指向用户目录。
源码运行及显式 `--root` 仍保持原 Portable 行为。冻结自重启须调用可执行文件，
不得用 `python -m vim2`；Windows 安装包不引入 GPU 依赖。
macOS 普通双击启动默认显示错误对话框；从 `/Volumes/.../*.app` 或标准
`/private/var/folders/.../AppTranslocation/<token>/d/.../*.app` 启动时只提示
拖到 Applications、弹出镜像后再启动，不请求临时身份的 TCC。安装后的首次普通 GUI 启动若缺少
辅助功能或输入监控，只能通过系统提示 API 请求并显示说明，不得绕过或代授予。
麦克风仍走系统录音流程。`--check` 和 `--import-smoke` 不弹权限请求。DMG 根目录必须
包含 UTF-8 `安装说明.txt`。

自包含打包使用 `packaging/vim2.spec`、`requirements-build.lock`、
`requirements-windows-cpu.lock`、`tools/build-macos-installer.sh`、
`tools/build-windows-installer.ps1` 和 `packaging/vim2.iss`，输出 `.sha256`。
没有品牌图标则保留默认图标，不虚构资产。macOS ad-hoc 签名不代表 Developer ID
或公证；Windows 未签名会遇到 SmartScreen；用户须自行核验 hash 和来源。
**对外分发前阻断：** 第三方 CPU 转换模型 HF card 当前缺 license metadata，
须核实模型来源/权利及许可证；另需两端真机安装、TCC/热键/剪贴板/音频和远程
Windows 粘贴验收。安装版只打包固定 Hugging Face 镜像
`csukuangfj2/sherpa-onnx-qwen3-asr-0.6B-int8-2026-03-25` revision
`68818b2313fe77bd06f6a7c5068ff3ef59d02b8a`；源码手册的 ModelScope
`zengshuishui/Qwen3-ASR-onnx` 只是同一转换的人工下载入口。首次用户文件必须
原子创建：临时文件写完并 fsync 后再替换，失败不得留下半截设置，且可重试。
下文早期三模型、GPU、Portable-only 与“不生成 EXE”描述
仅保留为源码 Portable 的历史要求，不再限制此安装版。

## 1. 背景与目标

VIM2 是 VIM 第一版的后续版本。产品继续提供 Windows 全局语音输入能力，并将识别模型升级为官方 Qwen3-ASR 模型。

### 1.1 产品目标

- 用户可在任意可输入文字的 Windows 应用中，通过全局热键开始和停止录音。
- 录音过程中显示实时转写预览，停止录音后生成最终结果并自动粘贴到原输入位置。
- 支持中文、英文及中英文混合语音，优先保证识别准确率。
- 内置 Qwen3-ASR 0.6B INT8 CPU、0.6B FP16 GPU 和 1.7B INT8 GPU，允许用户按场景切换。
- 所有语音识别均在本机执行，不上传音频或识别文本。
- 程序、配置和模型均从 Portable 目录运行；Python 及第三方包使用系统全局环境，不在项目目录中复制虚拟环境或依赖库。
- 整个应用使用 Python 实现，通过 `Start.cmd` 启动，不生成或依赖 EXE 程序。
- 系统托盘图标、状态配色、悬浮窗视觉风格、位置和动画沿用 VIM 第一版。

### 1.2 成功标准

- 在目标机器上，CPU 模式无需 CUDA 即可完成识别，两个 GPU 模式在 NVIDIA RTX A2000 Laptop 4GB 上均能完成识别且不发生显存不足。
- 三种模型的最终识别实时率均小于 1.0，即处理时间短于音频时长。
- 中英混合输入必须达到本文档定义的 CER、WER 和专有名词正确率门槛。
- 1.7B INT8 在中英混合、专有名词、长句或复杂语音测试集中应优于或至少不劣于 0.6B FP16。
- 用户无需安装 .NET、CUDA Toolkit、FFmpeg 或单独配置模型缓存；仅要求系统预装兼容版本的 Python 和锁定版本的第三方包，使用 GPU 模式时还需满足要求的 NVIDIA 驱动。
- 正式发布必须完成本文档第 6.5 节列出的全部范围。

### 1.3 非目标

- 不提供云端识别或在线 API 回退。
- 不要求说话人分离、翻译、情绪识别或音频事件识别。
- 不要求在多个模型之间同时并行推理。
- 不在本文档中规定具体 Python UI 框架、进程通信协议或推理后端。
- 本版本不提供图形化 A/B 测试界面。
- 本版本不提供集成在正式应用内的长期性能统计或诊断日志。
- 本版本不提供模型和运行环境完整性校验。
- 本版本不提供更丰富的输入法上下文或文本格式化功能。

### 1.4 macOS Apple Silicon Phase 1 边界

本文其余未特别标注的平台要求仍以 Windows 为准。macOS Phase 1 只允许原生
arm64 Python 3.11 和既有 `qwen3-asr-0.6b-int8-cpu` / sherpa-onnx CPU
后端；快速与高精度 CUDA 模型必须在平台预检处拒绝，不得静默 fallback。
模型仍从应用根目录的
`.models/sherpa-onnx-qwen3-asr-0.6B-int8-2026-03-25` 离线加载。

macOS 启动必须明确检查 Rosetta、Python 版本、模型完整性、CPU 与 PyObjC
原生依赖，并对辅助功能、输入监控和已拒绝/受限的麦克风权限给出可执行错误。
全局热键映射右 Option 到既有 RightAlt 语义；最终文本使用 NSPasteboard、恢复
目标应用，并按 macOS 配置发送 Command+V（默认、本机 Mac）或 Control+V
（Windows / 远程桌面）。Windows 的 Win32 mutex、热键、剪贴板、固定 Ctrl+V、
三个模型和启动脚本行为不得改变，且不引入 Command 键语义。

Phase 1 不声称已验证真实 M1 推理、TCC 全流程、应用兼容矩阵、签名、公证或
正式 `.app` 发布；这些项目必须在 Apple Silicon 真机与稳定签名身份下验收。

## 2. 用户流程

### 2.1 首次启动

1. 用户双击 Portable 目录中的 `Start.cmd`，由隐藏的 Windows PowerShell 启动 VIM2。
2. 程序验证 Python、模型和所选后端依赖是否可用；仅 GPU 模式验证 CUDA。
3. 程序加载上次选择的模型；仓库当前检入配置选择 0.6B INT8 CPU。Windows
   配置缺失或省略 `selected_model` 时仍按原行为回退到 Qwen3-ASR 0.6B FP16；
   macOS Phase 1 只允许 CPU。
4. 加载期间显示明确状态，不允许用户误以为程序已经可以录音。
5. 加载成功后程序进入系统托盘，并提示全局热键已经可用。

### 2.2 语音输入

1. 用户在目标应用中将光标放在输入位置。
2. 用户按一次全局热键开始录音。
3. 状态悬浮窗持续显示录音反馈；临时转写按配置的统一间隔发起，默认 1000 毫秒，可配置为 250–1000 毫秒。前一次推理未完成时只保留一个最新待处理请求，旧的待处理请求由更新快照覆盖。
4. 用户再次按下相同热键停止录音；程序必须在处理该热键时立即截止音频采集，不得等待实时预览或后台识别任务完成。
5. 程序封存完整录音；若预览确认了稳定完整句前缀，且带可配置安全重叠的尾段至少节省 30% 音频，则优先识别尾段并通过唯一精确句子锚点合并；否则或尾段识别失败时，使用完整录音识别。重叠默认 5 秒，可配置为 1–15 秒。
6. 最终结果替换悬浮窗中的临时预览，并自动粘贴到录音开始前的目标窗口。
7. 程序恢复待机状态。

### 2.3 模型切换

1. 用户右键单击系统托盘图标。
2. 用户从“识别模型”子菜单中选择 CPU 模式或两个 GPU 模式之一。
3. 程序停止并卸载当前模型，释放对应的内存或显存资源。
4. 程序加载新模型，并在托盘和悬浮窗中显示加载状态。
5. 新模型加载成功后恢复录音能力。
6. 用户选择应持久化，在下次启动时继续使用。

任意时刻只允许一个 ASR 模型驻留内存和显存。

只有“就绪”状态允许切换模型。录音、实时识别、最终识别期间，以及存在等待重试的音频时，模型菜单必须禁用。重试必须继续使用失败时选中的模型；用户取消录音或放弃待重试音频后才能切换。

### 2.4 macOS 粘贴快捷键切换

1. macOS 用户从托盘“粘贴快捷键”子菜单选择“macOS：⌘V”或
   “Windows / 远程桌面：Ctrl+V”。
2. 两个选项互斥，当前生效项必须显示选中标记。
3. 选择成功后立即用于后续粘贴，写入 Portable 配置并在下次启动恢复。
4. 仅“就绪”状态允许切换；录音、实时识别、最终识别、等待重试、模型切换等
   状态必须禁用菜单，避免会话中途改变。
5. 持久化失败时必须显示错误，并继续使用和显示此前有效选择；运行时不使用可抛出
   异常的回调更新或二次回滚。
6. Windows 不显示此菜单，Windows 本机粘贴仍固定使用 Ctrl+V。

## 3. 功能需求

### 3.1 全局热键

- 支持单键和组合键。
- 支持区分左右 Ctrl、Alt、Shift 和 Win 键。
- 默认热键沿用 VIM 第一版的 Right Alt。
- 第一次触发开始录音，第二次触发停止录音。
- 录音过程中按 Esc 取消，取消后不得识别或粘贴任何文字。
- 热键配置保存在 Portable 目录中。
- 每次完整的按下并释放只触发一次，系统按键自动重复不得重复触发。
- 通用 `Ctrl`、`Alt`、`Shift`、`Win` 匹配对应的任意左键或右键；`Left*` 和 `Right*` 只匹配指定侧。
- 组合键中的所有配置键都按下时触发；额外按下未配置的键不得导致触发。
- 热键触发后应吞掉构成该热键的按键事件，避免向目标应用输入无关字符。
- 默认 Right Alt 在完全释放后才允许发送最终结果对应的 Ctrl+V。

### 3.2 麦克风录音

- 每次开始录音时使用 Windows 当前的系统默认输入设备。
- 本版本不提供麦克风设备选择功能。
- 用户在 Windows 系统设置中切换默认麦克风后，下一次录音应使用新的默认设备，无需修改 VIM2 配置。
- 录音开始、停止和设备错误必须有明确反馈。
- 音频输入 overflow 作为非致命警告显示；不得丢弃仍可识别和粘贴的已录音频。
- 默认最长录音时间为 300 秒；达到上限后自动停止并识别。
- 不得因输入设备暂时不可用而无提示退出。

### 3.3 实时转写

- 临时文本使用统一的预览间隔，默认 1000 毫秒，可通过 Portable 配置设为 250–1000 毫秒；推理任务串行执行，最多保留一个最新待处理请求，悬浮窗必须持续提供录音状态反馈。
- 临时文本必须由当前选中的 Qwen3-ASR 模型生成，不允许使用另一个轻量模型替代。
- 允许临时文本随上下文增加而回改。
- 临时文本必须在视觉上与已提交文本区分，不应直接反复写入目标应用。
- 实时转写失败时应继续保留录音，并在停止后尝试完整语句识别。
- 实时转写不得同时创建多个模型实例。
- 用户停止录音、取消录音或退出时，必须清除待处理预览并请求取消正在运行的预览；尚未进入模型的任务不得执行，已经进入生成阶段的任务应尽快停止，取消产生的部分文本不得显示或提交。
- 单次实时预览只处理最近 `preview_window_seconds` 秒音频（默认 8 秒，可配置为 1–30 秒），避免长录音使预览无限增长并阻塞最终识别；存在稳定完整句前缀和唯一精确锚点时可合并显示完整预览，否则允许只显示最近窗口的文本。
- 本需求只约束可配置且最长一秒的统一调度间隔、同一模型和可回改等可观察行为，不限定流式 API 的具体实现方式。

### 3.4 最终识别

- 实时预览不得直接作为最终结果提交；停止录音后必须以内存中封存的完整音频或从该音频切分的尾段完成最终识别，不得为录音、预览或尾段生成本地 WAV 文件。
- 多句预览立即确认最后一个完整句之前的内容；只有一个完整句时，连续三次成功预览完全一致后才稳定。完整句末标点仅包括 `。`、`！`、`？`、`.`、`!` 和 `?`，预览失败不计入连续次数，稳定前缀只能增长。
- 存在稳定前缀时，从检查点前保留可配置的安全重叠，默认 5 秒，可配置为 1–15 秒；只有尾段比完整录音至少短 30% 时才允许尾段优化。
- 尾段文本必须恰好包含一次与稳定前缀最后一个完整句完全一致的锚点，才能与稳定前缀合并；锚点缺失、重复、尾段为空、节省不足或尾段识别发生预期错误时，必须识别完整音频。
- 最终结果不得使用模糊匹配或简单拼接实时片段。
- 自动检测中文、英文及中英文混合内容。
- 最终输出中，相邻的不同文字体系之间必须保留一个空格；数字和标点不触发该规则，日文汉字与假名视为同一兼容体系。
- 最终识别完成前不得向目标窗口粘贴不稳定文本。
- 空结果不得覆盖剪贴板或触发粘贴。
- 识别失败时显示错误，并允许用户重试；不得返回伪造或成功形态的空结果。
- 识别失败后应在内存中保留该次音频，直到重试成功、用户主动放弃或程序退出；随后立即释放。
- 用户重试时必须继续使用产生失败的模型并识别完整音频，不得再次执行尾段优化。

### 3.5 自动粘贴

- 最终文本应粘贴到录音开始时处于前台的目标窗口。
- Windows 粘贴行为沿用 VIM 第一版的剪贴板与 SendInput 机制：程序将最终结果以
  Unicode 文本写入系统剪贴板，恢复录音开始时记录的目标窗口，然后固定发送一次
  Ctrl+V；macOS 选项不得改变 Windows 行为或为 Windows 引入 Command 键语义。
- macOS 使用 NSPasteboard 并恢复、复核录音开始时的目标应用；本机 Mac 默认发送
  Command+V，Windows / 远程桌面场景可发送 Control+V。两种事件序列都必须携带
  VIM2 marker，并在部分注入失败时安全释放 V 键和修饰键。
- 粘贴后不恢复用户原有剪贴板内容；最终识别文本保留在剪贴板中。
- 兼容常规桌面应用和远程桌面场景。
- 粘贴失败时显示明确错误，不得静默丢失识别结果。
- 即使粘贴失败，用户仍应能从界面或剪贴板取回最终文本。

### 3.6 系统托盘

托盘菜单至少包含：

- 当前状态。
- 识别模型。
  - Qwen3-ASR 0.6B INT8。
  - Qwen3-ASR 0.6B FP16。
  - Qwen3-ASR 1.7B INT8。
- macOS 专用的“粘贴快捷键”子菜单。
  - macOS：⌘V。
  - Windows / 远程桌面：Ctrl+V。
- 打开热词文件。
- 重新加载热词。
- 关于。
- 退出。

当前模型必须以选中标记显示。
托盘菜单和托盘图标单击均不得开始或停止录音；录音仅由全局热键控制。

托盘图标沿用 VIM 第一版的 16×16 抗锯齿圆点设计和状态颜色：

| 状态 | RGB |
|---|---|
| 加载中 | `150, 150, 150` |
| 就绪 | `60, 180, 75` |
| 录音中 | `220, 50, 50` |
| 识别中 | `230, 180, 30` |

- 圆点绘制区域为 12×12，并位于 16×16 透明画布中央。
- 托盘提示文字继续使用 VIM 第一版的 Loading、Ready、Recording 和 Processing 状态语义。
- 新增的模型切换菜单不得改变托盘图标的基本视觉风格。

### 3.7 状态与错误处理

程序至少区分以下状态：

- 启动中。
- 模型加载中。
- 就绪。
- 录音中。
- 实时识别中。
- 最终识别中。
- 模型切换中。
- 错误。

模型加载失败、CUDA 不可用、显存不足、麦克风不可用和粘贴失败必须显示可执行的错误信息。
错误悬浮提示显示 10 秒后自动淡出并移到屏幕外；后续成功粘贴完成时应立即淡出。

### 3.8 热词与上下文偏置

- 热词文件位于 Portable 目录的 `config/hotwords.txt`，使用 UTF-8 编码，一行一个热词或短语；文件缺失时自动生成带说明注释的空模板。
- 解析接受 BOM，去除首尾空白，忽略空行和以 `#` 开头的注释行，并对完全相同的项目保序去重。
- 最多加载 100 项，每项最多 100 个字符。项目不得包含 ASCII 逗号，避免破坏 CPU 后端的热词分隔格式。
- GPU 后端把有效项目按换行拼接为 qwen-asr context；CPU 后端按 ASCII 逗号拼接为 sherpa-onnx hotwords。
- 模型启动或切换时加载热词。托盘提供“打开热词文件”和“重新加载热词”入口。
- Windows 的“打开热词文件”启动并跟踪记事本进程；仅在该进程关闭后比较文件修改时间，修改时间发生变化时自动刷新，编辑期间不得刷新。macOS 使用系统默认关联程序打开文件，编辑后使用托盘手动重新加载热词。
- GPU 刷新只替换不可变 context snapshot，不重载模型权重；CPU 刷新重建 recognizer。
- 录音、实时识别、最终识别和等待重试期间不得刷新，确保一次录音始终使用同一 snapshot。
- 刷新失败时保留上一次成功 snapshot 并明确提示用户。热词仅为 soft bias，不执行字符串后处理替换。

## 4. 模型与 Portable 交付要求

### 4.1 内置模型

发布包必须预置：

| 显示名称 | 模型 | 精度 | 默认 |
|---|---|---|---|
| CPU 模型 | Qwen3-ASR 0.6B ONNX | INT8 | 否 |
| 快速模型 | Qwen3-ASR 0.6B | FP16 | 是（Windows 字段缺失时） |
| 高精度模型 | Qwen3-ASR 1.7B | INT8 | 否 |

“默认”列描述 Windows 的缺省回退语义；仓库当前检入配置选择 CPU。Windows 可
选择全部三个模型，macOS Phase 1 只允许 CPU。

- GPU 模式必须使用官方 Qwen3-ASR 权重。
- 如交付预量化权重，应记录来源、原始 revision、量化方法和校验值。
- 不得把第三方转换模型冒充为官方原始权重。
- 发布包必须附带机器可读的版本清单，锁定 Python 包、推理后端、模型 revision、量化配置及文件校验值。

当前 POC 的可复现基线为：

| 项目 | 固定值 |
|---|---|
| 0.6B 模型 ID | `Qwen/Qwen3-ASR-0.6B` |
| 0.6B revision | `5eb144179a02acc5e5ba31e748d22b0cf3e303b0` |
| 0.6B 精度 | FP16 |
| 1.7B 模型 ID | `Qwen/Qwen3-ASR-1.7B` |
| 1.7B revision | `7278e1e70fe206f11671096ffdd38061171dd6e5` |
| 1.7B 量化 | `BitsAndBytesConfig(load_in_8bit=True)` |
| qwen-asr | `0.0.6` |
| Transformers | `4.57.6` |
| PyTorch | `2.11.0+cu128` |
| bitsandbytes | `0.48.2` |

实现可以升级以上版本或更换等价 INT8 实现，但必须同步更新版本清单，并重新执行完整准确率、性能、显存和离线验收。

### 4.2 模型生命周期

- 程序启动时只加载用户选中的模型。
- Windows 配置省略 `selected_model` 时默认主流程仍使用 0.6B FP16；仓库当前检入配置选择 CPU。1.7B INT8 仅在 Windows 用户主动选择高精度模式后使用，macOS Phase 1 不允许该模型。
- 常规语音输入期间模型保持驻留，避免每次识别重复加载。
- 切换模型时必须卸载旧模型并释放 GPU 后再加载新模型。
- 若新模型加载失败，应显示错误并允许恢复到上一个可用模型。
- A/B 测试工具应继续采用独立进程方式，确保各模型的显存测量互不影响。

### 4.3 Portable 目录

除 Python 外，所有运行内容必须位于应用目录：

```text
VIM2/
├── Start.cmd
├── start.ps1
├── app/
│   └── Python 应用代码
├── requirements.lock
├── .models/
│   ├── Qwen3-ASR-0.6B/
│   └── Qwen3-ASR-1.7B-INT8/
├── config/
│   ├── settings.json
│   └── hotkey.conf
└── tools/
    └── A/B 测试工具
```

- 正常运行不得依赖用户目录中的 Hugging Face 或 ModelScope 缓存。
- 正常运行不得自动联网下载模型。
- 发布准备时必须从本机现有 Hugging Face 缓存复制模型到 `VIM2\.models`，不得重新从网络下载。
- 复制完成后必须保留原缓存文件，不得移动、重命名或删除原模型。
- 如果目标模型目录已经完整存在，不得重复复制或覆盖。
- 当前模型源目录为：
  - `C:\Users\keynesy\.cache\huggingface\hub\models--Qwen--Qwen3-ASR-0.6B`，约 1.75 GB。
  - `C:\Users\keynesy\.cache\huggingface\hub\models--Qwen--Qwen3-ASR-1.7B`，约 4.38 GB。
- 应从各模型 `snapshots\<revision>` 目录复制完整快照到对应的 Portable 模型目录；不得仅复制 Hugging Face 缓存中的单个 blob 文件。
- 移动整个 VIM2 目录后，除全局 Python 环境及其第三方包外不需要重新配置。
- 路径中包含空格或中文时必须正常运行。
- 配置、日志和模型路径均应以应用目录为基准。
- 发布说明必须列出支持的 Python 大版本、最低 NVIDIA 驱动版本和实际验证过的 Windows 版本。
- `Start.cmd` 和 `start.ps1` 必须从自身所在目录解析所有相对路径，不得依赖用户当前工作目录。
- `Start.cmd` 必须通过隐藏的 Windows PowerShell 和 `pythonw.exe` 启动，不得在桌面保留命令行窗口。
- 启动脚本必须检查 Python 是否存在且版本兼容；图形启动失败时使用消息框显示明确错误。诊断入口使用 `python -m vim2 --check` 并保留控制台输出。
- Python 第三方依赖使用全局 Python 环境，版本由根目录 `requirements.lock` 锁定；正常启动只验证依赖，不得执行 `pip install`。
- 正式应用的业务逻辑、托盘、悬浮窗、全局热键、录音、模型管理和自动粘贴均使用 Python 实现。

`config/settings.json` 支持以下字段：

| 字段 | 类型 | 默认值 | 有效范围或取值 |
|---|---|---:|---|
| `selected_model` | 字符串 | `qwen3-asr-0.6b-fp16`（Windows 字段缺失回退） | `qwen3-asr-0.6b-int8-cpu`（CPU）、`qwen3-asr-0.6b-fp16`（FAST）、`qwen3-asr-1.7b-int8`（ACCURATE）；macOS Phase 1 仅 CPU |
| `max_recording_seconds` | 整数 | `300` | 1–300 |
| `preview_interval_ms` | 整数 | `1000` | 250–1000 |
| `preview_window_seconds` | 整数 | `8` | 1–30；单次实时预览最多使用的最近音频 |
| `tail_overlap_seconds` | 整数 | `5` | 1–15；稳定检查点之前保留的重叠音频 |
| `macos_paste_shortcut` | 字符串 | `command-v` | `command-v`（本机 Mac 的 ⌘V）或 `control-v`（Windows / 远程桌面的 Ctrl+V）；仅影响 macOS |
| `normalize_numbers` | 布尔值 | `true` | `true`、`false`；控制可移除的中英文数字后处理器 |

缺少可选字段时使用默认值；字段类型或范围无效时，启动检查必须报告明确错误。
`macos_paste_shortcut` 缺失时兼容既有配置并使用 `command-v`；非法值必须拒绝，
不得静默回退。选择变化必须只对 `settings.json` 做单文件原子替换，不得重写
`hotkey.conf`；原子替换成功后才更新共享运行时选择和菜单，失败时磁盘、运行时与
菜单均保持旧值。本地配置文件若覆盖此字段，托盘切换应明确拒绝，防止重启后本地值
覆盖新选择。Windows 本机始终使用 Ctrl+V。
当前检入的 `config/settings.json` 明确选择 `qwen3-asr-0.6b-int8-cpu`，因此不触发
Windows 的字段缺失回退。

### 4.4 离线与隐私

- 模型准备完成后，断网环境下必须可启动和识别。
- 不得上传音频、识别文本、设备信息或性能数据。
- 如提供更新检查，必须默认关闭或明确征得用户同意。

## 5. 界面原型

### 5.1 系统托盘菜单

```text
┌──────────────────────────────────────┐
│ VIM2：就绪                           │
├──────────────────────────────────────┤
│ 识别模型                          ▶  │
│   ✓ CPU：Qwen3-ASR 0.6B INT8         │
│     快速：Qwen3-ASR 0.6B FP16        │ ← Windows 专用
│     高精度：Qwen3-ASR 1.7B INT8      │ ← Windows 专用
│ 粘贴快捷键                        ▶  │ ← 仅 macOS
│   ✓ macOS：⌘V                        │
│     Windows / 远程桌面：Ctrl+V       │
├──────────────────────────────────────┤
│ 打开热词文件                         │
│ 重新加载热词                         │
├──────────────────────────────────────┤
│ 关于                                 │
│ 退出                                 │
└──────────────────────────────────────┘
```

### 5.2 录音悬浮窗

```text
┌──────────────────────────────────────────────┐
│ ● 正在录音                                   │
│ 我们需要把 Kubernetes deployment 更新到……   │
└──────────────────────────────────────────────┘
```

- 红色状态点表示正在录音。
- 显示当前模型产生的临时文本。
- 临时文本长度超过窗口时始终显示最新的末尾内容，但完整文本应保留在内部状态中。
- 悬浮窗不得抢占目标应用焦点。
- 悬浮窗沿用 VIM 第一版的深色半透明圆角胶囊风格：
  - 背景色 `#E0191920`。
  - 边框色 `#50505064`，边框宽度 1。
  - 圆角半径 14。
  - 文字色 `#F0F0F5`，Segoe UI 12。
  - 录音圆点颜色 `#E63C3C`，尺寸 10×10。
- 悬浮窗位于目标窗口所在显示器工作区的底部中央，距离底部 60 个逻辑像素。
- 悬浮窗保持置顶、从任务栏隐藏、鼠标穿透且永不获取焦点。
- 悬浮窗不得使用系统窗口阴影；宽度应随内容在紧凑范围内变化，并在宽度变化后保持屏幕底部居中。
- 显示和隐藏继续使用约 200 毫秒淡入淡出。
- 录音红点继续以约 50 毫秒间隔在 0.3 到 1.0 透明度之间脉动。
- 高度保持 32 个逻辑像素；宽度可从第一版的 110 扩大以容纳实时文字，但整体仍保持单行紧凑胶囊外观。

### 5.3 最终识别状态

```text
┌──────────────────────────────────────────────┐
│ ● 正在录音                                   │
│ 我们需要把 Kubernetes deployment 更新到……   │
└──────────────────────────────────────────────┘
```

- 最终识别期间继续保留最后一次实时识别文本，不使用计算状态文字覆盖它。

### 5.4 模型切换状态

```text
┌──────────────────────────────────────────────┐
│ 正在切换到高精度模型                         │
│ Qwen3-ASR 1.7B INT8                          │
└──────────────────────────────────────────────┘
```

切换期间托盘菜单不得允许重复触发模型切换，全局热键不得开始录音。

## 6. 性能、可靠性与验收标准

### 6.1 目标设备

最低验收设备：

```text
Windows 10/11 x64
NVIDIA RTX A2000 Laptop GPU，4GB 显存
32GB 系统内存
兼容版本 Python
```

### 6.2 性能要求

- 0.6B FP16 最终识别 RTF 必须小于 1.0。
- 0.6B INT8 CPU 最终识别 RTF 必须小于 1.0，且不要求 CUDA/NVML 显存指标。
- 1.7B INT8 最终识别 RTF 必须小于 1.0。
- 实时临时文本按统一配置间隔请求，默认 1000 毫秒，可配置为 250–1000 毫秒；前一次推理未完成时不排队，完成后等待下一次定时请求，不得立即连续补跑。
- 单次实时预览只复制并识别最近的 `preview_window_seconds`，默认 8 秒；稳定句子前缀用于边界合并，`tail_overlap_seconds` 决定检查点前保留的重叠音频。
- 相同的临时文本不得重复触发悬浮窗重绘；推理请求频率与界面更新频率相互独立。
- CPU 后端采用协作式取消；同步 `decode_stream()` 无法安全地从 Python 中途终止，取消延迟由有界预览窗口控制。
- Windows 缺省 0.6B FP16 模型开始录音后应尽快显示第一段非空临时文本；在目标设备和固定验收语料上，存在新语音时的预览完成间隔 P95 目标不超过 3 秒。macOS CPU 模型须在真机另行建立基线。
- 实时识别不得导致录音丢帧或 UI 卡死。
- 0.6B FP16 峰值显存不得超过 2.5 GiB。
- 1.7B INT8 峰值显存不得超过 3.8 GiB。
- 模型切换完成后，旧模型对应的系统内存或 CUDA 资源必须释放。
- 性能统计必须区分模型加载时间和纯推理时间。

历史短音频 POC 基线：

| 模型 | 音频 | 推理时间 | RTF | 峰值显存 |
|---|---:|---:|---:|---:|
| 0.6B FP16 | 15.05 秒 | 6.17 秒 | 0.410 | 1.75 GiB |
| 1.7B INT8 | 15.05 秒 | 19.33 秒 | 1.284 | 2.81 GiB |

2026-09-14 使用带背景音乐的 44.745 秒音轨得到的当前基线：

| 模型 | 音频 | 推理时间 | RTF | 峰值显存 |
|---|---:|---:|---:|---:|
| 0.6B FP16 | 44.745 秒 | 9.97 秒 | 0.223 | 1.80 GiB |
| 1.7B INT8 | 44.745 秒 | 44.66 秒 | 0.998 | 2.81 GiB |

同一音轨的 1.7B 尾段耗时为：4 秒音频 4.22 秒、8 秒音频 6.77 秒、12 秒音频 9.82 秒。149.235 秒长录音的 latest-wins 模拟中，0.6B 平均刷新间隔为 2.52 秒，1.7B 为 10.20 秒。因此 0.6B FP16 仍是 Windows 字段缺失时的缺省主模型；仓库当前检入配置选择 CPU。latest-wins 用于限制积压，不用于掩盖模型本身的推理延迟。

性能验收统一采用以下方法：

- 使用固定版本的验收语料和版本清单。
- 模型预先加载并完成一次不计分的预热识别。
- 每个模型单独启动测试进程；GPU 模式测试前确认无其他 GPU 计算任务。
- 每条音频连续测试 3 次，记录中位数。
- 最终 RTF 按“全部纯推理时间之和 / 全部音频时长之和”计算，不包含模型加载和文件下载。
- 同时单独报告冷启动模型加载时间。
- GPU 模式峰值显存以进程级 CUDA/NVML 测量为准；所有模式记录 Python、推理后端和量化版本，GPU 模式另记录驱动和 PyTorch 版本。
- 本版本必须提供独立的命令行 A/B 验收工具，输出加载时间、纯推理时间、RTF、峰值显存和原始文本。

### 6.3 准确率验收

建立不少于 100 条真实录音的固定测试集，至少包含：

- 中文短句。
- 连续英文。
- 同一句内多次中英文切换。
- 英文技术术语和产品名。
- 数字、日期、金额、单位和缩写。
- 普通话口音、轻声和吞音。
- 安静、键盘声和一般室内噪声。
- 5 秒以内短句和 30 秒以上长句。

记录以下指标：

- 中文字符错误率 CER。
- 英文词错误率 WER。
- 中英文切换边界错误数。
- 专有名词完全正确率。
- 首次临时文本延迟。
- 临时文本回改率。
- 最终识别 RTF。
- 峰值显存和内存。

验收要求：

- 测试集必须保存人工校对的参考文本，并冻结为带版本号的验收数据集。
- 分别报告中文 CER、英文 WER、中英文边界错误数和专有名词完全正确率，不使用含义不明确的单一“综合准确率”。
- 两模型的中文 CER 均不得高于 5%。
- 两模型的英文 WER 均不得高于 10%。
- 两模型的专有名词完全正确率均不得低于 90%。
- 中英文切换边界错误句数不得超过中英混合测试句总数的 5%。
- 1.7B INT8 的中文 CER 和英文 WER均不得高于 0.6B FP16，专有名词完全正确率不得降低。
- VIM 第一版可作为附加对照组记录，但不作为缺少历史基线时的发布前置条件。
- 对两模型输出不一致的样本必须保留音频、参考文本和两份结果，支持人工复核。
- 性能测试必须在相同音频、相同设备和无其他 GPU 工作负载的条件下执行。

### 6.4 可靠性要求

- 连续完成 100 次开始、停止、识别循环，不崩溃、不泄漏模型实例。
- 连续切换模型 20 次，不发生显存持续增长。
- 麦克风断开后给出错误，重新连接后可恢复。
- 模型失败后热键、托盘和录音状态必须恢复一致。
- 单实例运行，重复启动时提示用户。
- 退出程序后不得残留 ASR Worker 或 GPU 显存占用。
- 内存中的完整录音和尾段切片按最终识别章节定义的成功、放弃或退出条件释放。
- 连续录音期间，音频回调不得报告输入溢出；最终采集帧数与采样率计算出的录音时长和墙钟时长误差不得超过 1%。
- 模型推理期间每 100 毫秒探测一次 UI 消息响应；连续无响应时间不得超过 500 毫秒。
- 自动粘贴至少在 Windows 记事本、Microsoft Word、主流浏览器文本框、Visual Studio Code 和一次远程桌面会话中各连续测试 20 次，成功率必须为 100%。

### 6.5 本版本交付范围

- Portable 目录和离线模型加载。
- 0.6B INT8（复用官方 0.6B 权重，运行时动态量化）。
- 0.6B FP16。
- 1.7B INT8。
- 全局热键录音。
- 同模型实时转写。
- 停止时取消过时预览，并使用封存音频的尾段或完整音频生成最终结果。
- 自动粘贴。
- 托盘状态和错误提示。
- 托盘模型切换。
- 命令行 A/B 性能与准确率验收工具。
